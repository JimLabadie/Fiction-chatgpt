...
**Persistence and downstream application require independent fidelity gates.** A persistent record may not certify its own completeness. When development, correction, approval, or source work occurs in conversation or another upstream source, verify that the controlling repository destination preserves that upstream material at full established resolution before using the destination as the sole audit basis. Then independently verify that downstream prose or analysis preserves the controlling record. For story prose, also verify reader-visible meaning and voice/narrative function. Retrieval, a commit, a source receipt, or a matching list of nouns does not by itself prove fidelity. The controlling enforcement procedure is maintained in `system bible/01-system-and-canon-framework/persistence-failure-and-repair-record.md` and the story-prose gates in `system bible/00-mandatory-source-grounded-drafting.md`.
